 ...
 */
func calculeteArea(x: Double, y: Double) -> Double? {
    guard x > 0, y > 0 else {
        return nil
    }
    return x * y
}
calculeteArea(x: 10, y: -10)
...
func createUser() -> User? {
    guard let fName = firstNameTextField.text, let sName = lastNameTextField.text, let ageUser = ageTextField.text else {
        return nil
    }
    return User(firstName: fName, lastName: sName, age: ageUser)
}
...
if let user = userOne {
    print("I'm \(user.firstName) \(user.lastName), and i'm \(user.age) year's old.")
}
/*:
